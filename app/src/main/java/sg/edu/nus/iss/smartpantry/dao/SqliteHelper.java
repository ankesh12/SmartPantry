...
public class SqliteHelper extends SQLiteOpenHelper {
    ...
    public static final String COL_SHOPPING_LIST_PRODUCT_NAME = "ProductName";
    public static final String COL_SHOPPING_LIST_CATEGORY_NAME = "CategoryName";
    ...
    private final String CREATE_TABLE_ITEM = "CREATE TABLE "+TABLE_ITEM+" ("+COL_ITEM_ID+" INTEGER (5), "+COL_ITEM_PRODUCT_NAME+" STRING NOT NULL ON CONFLICT ROLLBACK, "+COL_ITEM_CATEGORY_NAME+" STRING NOT NULL ON CONFLICT ROLLBACK, "+COL_ITEM_EXPIRY_DATE+" DATE, "+COL_ITEM_PRICE+" DECIMAL (4, 2) NOT NULL ON CONFLICT ROLLBACK DEFAULT (0), "+COL_ITEM_DOP+" DATE DEFAULT (CURRENT_DATE) NOT NULL ON CONFLICT ROLLBACK, PRIMARY KEY ("+COL_ITEM_ID+", "+COL_ITEM_PRODUCT_NAME+", "+COL_ITEM_CATEGORY_NAME+") ON CONFLICT ROLLBACK, FOREIGN KEY ("+COL_ITEM_PRODUCT_NAME+", "+COL_ITEM_CATEGORY_NAME+") REFERENCES "+TABLE_PRODUCT+" ("+COL_PROD_NAME+", "+COL_PROD_CATEGORY_NAME+") ON DELETE CASCADE ON UPDATE CASCADE MATCH SIMPLE)";
    private final String CREATE_TABLE_SHOPPING_LIST="CREATE TABLE "+TABLE_SHOPPING_LIST+" ("+COL_SHOPPING_LIST_NAME+" STRING, "+COL_SHOPPING_LIST_PRODUCT_NAME+" STRING NOT NULL ON CONFLICT ROLLBACK, "+COL_SHOPPING_LIST_CATEGORY_NAME+" STRING NOT NULL ON CONFLICT ROLLBACK, "+COL_SHOPPING_LIST_QTY+" INTEGER DEFAULT (1), "+COL_SHOPPING_LIST_IS_PURCHASED+" BOOLEAN DEFAULT (0), FOREIGN KEY ("+COL_SHOPPING_LIST_PRODUCT_NAME+", "+COL_SHOPPING_LIST_CATEGORY_NAME+") REFERENCES "+TABLE_PRODUCT+" ("+COL_PROD_NAME+", "+COL_PROD_CATEGORY_NAME+") ON DELETE CASCADE ON UPDATE CASCADE MATCH SIMPLE, PRIMARY KEY("+COL_SHOPPING_LIST_NAME+", "+COL_SHOPPING_LIST_PRODUCT_NAME+", "+COL_SHOPPING_LIST_CATEGORY_NAME+") ON CONFLICT ROLLBACK)";
